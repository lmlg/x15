...
// An alert as generated by a kernel event (e.g: a task died).
...
cap_flow_guard_make (struct cap_flow *flow)
{
  ...
  return (spinlock_guard_make (&flow->alerts.lock, save_intr));
}
...
{
  memset (alert->payload, 0, sizeof (alert->payload));
...
cap_alert_free (struct cap_alert *alert)
{
  ...
  else if (type == CAP_ALERT_THREAD_DIED || type == CAP_ALERT_TASK_NOTIF)
...
cap_flow_fini (struct sref_counter *sref)
{
  ...
  pqueue_for_each_entry_safe (&flow->alerts.pending, alert, tmp, pnode)
    if (cap_alert_type (alert) == CAP_ALERT_USER)
      kmem_cache_free (&cap_misc_cache, alert);

  hlist_for_each_entry_safe (&flow->alerts.alloc, alert, tmp, hnode)
    cap_alert_free (alert);

  for (_Auto lpad = flow->lpads.free_list; lpad; )
    {
      _Auto next = lpad->next;
                 ...
                 uintptr_t tag, uintptr_t entry)
{
  ...
  hlist_init (&ret->alerts.alloc);
  pqueue_init (&ret->alerts.pending);

  ret->lpads.free_list = NULL;
  list_init (&ret->lpads.waiters);
  spinlock_init (&ret->lpads.lock);
...
{
  if (list_empty (&flow->alerts.receivers))
    return;
                  ...
                  uint32_t flags, uint32_t prio)
{
...

  {
    ...
    recv = list_pop (&flow->alerts.receivers, typeof (*recv), lnode);
  }
...
cap_flow_push_lpad (struct cap_flow *flow, struct cap_lpad *lpad)
{
  while (1)
    {
      _Auto next = lpad->next = atomic_load_rlx (&flow->lpads.free_list);
      if (!atomic_cas_bool_rel (&flow->lpads.free_list, next, lpad))
        {
          atomic_spin_nop ();
          continue;
        }
      ...
      atomic_fence_acq ();
      if (!next || !list_empty (&flow->lpads.waiters))
        {
          SPINLOCK_GUARD (&flow->lpads.lock);
          if (list_empty (&flow->lpads.waiters))
            return;

          _Auto sn = list_first_entry (&flow->lpads.waiters,
                                       struct cap_sender, lnode);
          thread_wakeup (sn->thread);
        }
...
cap_lpad_pop_free (struct cap_lpad **ptr)
{
  RCU_GUARD ();
  while (1)
    {
...
cap_alert_async_find (struct cap_flow *flow, int type, int id)
{
  ...
  hlist_for_each_entry (&flow->alerts.alloc, tmp, hnode)
...
cap_intr_register (struct cap_flow *flow, uint32_t irq)
{
  ...
  hlist_insert_head (&flow->alerts.alloc, &ap->base.hnode);
  cap_flow_guard_fini (&guard);
  return (0);
}
                     ...
                     uint32_t id, struct cap_alert_async **outp)
{
  ...
  if (!pqueue_node_unlinked (&entry->base.pnode))
    pqueue_remove (&flow->alerts.pending, &entry->base.pnode);
                          ...
                          uint32_t prio, int type, struct bulletin *outp)
{
  ...
  list_rcu_insert_tail (&outp->subs, &ap->xlink);
...
void
cap_notify (struct bulletin *bulletin, uint32_t flags)
{
  struct cap_alert_async *ap;
  list_rcu_for_each_entry (&bulletin->subs, ap, xlink)
    {
      _Auto flow = ap->flow;

      CAP_FLOW_GUARD (flow);
      ap->base.k_alert.task.flags |= flags;
