...
{
  assert (max_msg_size);
  unsigned int order = (sizeof (max_msg_size) * CHAR_BIT) -
                       __builtin_clzl (max_msg_size);
...
mbuf_clear_old_msgs (struct mbuf *mbuf, size_t total_size)
{
  ...
  size_t hdr_size = mbuf_compute_hdr_size (mbuf->order);
  void *msg_size_addr = mbuf_hdr_get_msg_size_addr (&hdr, mbuf->order);

  do
    {
      size_t size = hdr_size;
      int error = cbuf_pop (&mbuf->cbuf, msg_size_addr, &size);
      assert (! error);

      if (! size)
        break;

      size_t msg_size = mbuf_hdr_get_msg_size (&hdr, mbuf->order);
      size = msg_size;
...
mbuf_push (struct mbuf *mbuf, const void *buf, size_t size, bool erase)
{
  ...
  size_t total_size = mbuf_hdr_get_total_msg_size (&hdr, mbuf->order);
  ...
  int error = cbuf_push (&mbuf->cbuf, msg_size_addr, hdr_size, erase);
  assert (! error);
...
{
  size_t start = cbuf_start (&mbuf->cbuf);
  int error = mbuf_read (mbuf, &start, buf, sizep);
...
mbuf_read (const struct mbuf *mbuf, size_t *indexp, void *buf, size_t *sizep)
{
  ...
  size_t size = hdr_size;
  int error = cbuf_read (&mbuf->cbuf, *indexp, msg_size_addr, &size);
