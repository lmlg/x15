...
cga_get_cursor_position (void)
{
  ...
  uint16_t tmp = io_read_byte (CGA_PORT_CRTC_DATA) << 8;
...
cga_enable_cursor (void)
{
  ...
  uint8_t tmp = io_read_byte (CGA_PORT_CRTC_DATA);
...
{
  uint16_t cursor = cga_bbuf_cursor_offset (bbuf);
  assert (!(cursor & 1));
...
{
  uint16_t cursor = 0;
  bool cursor_enabled = !cga_bbuf_get_phys_cursor (bbuf, &cursor);
...
{
  size_t size = CGA_MEMORY_SIZE;
  int error = cbuf_read (&bbuf->cbuf, bbuf->view, cga_memory, &size);
  assert (!error);
...
{
  size_t view_size = cbuf_end (&bbuf->cbuf) - bbuf->view;
  return (view_size > CGA_MEMORY_SIZE ||
          // Consider the cursor as a valid cell.
          bbuf->cursor + 1 - bbuf->view > CGA_MEMORY_SIZE);
}
...
cga_bbuf_push (struct cga_bbuf *bbuf, char c)
{
  ...
  uint16_t cell = cga_build_cell (c);
  cbuf_write (&bbuf->cbuf, bbuf->cursor, &cell, sizeof (cell));

  size_t offset = cga_bbuf_cursor_offset (bbuf);
...
cga_bbuf_newline (struct cga_bbuf *bbuf)
{
  ...
  int error = cga_bbuf_get_phys_cursor (bbuf, &cursor);
  assert (!error);

  size_t nr_spaces = CGA_COLUMNS - (cursor % CGA_COLUMNS);
...
cga_bbuf_scroll_up (struct cga_bbuf *bbuf)
{
  ...
  size_t size = cbuf_size (&bbuf->cbuf);
  size -= size % CGA_MEMORY_LINE_SIZE;
  size_t start = cbuf_end (&bbuf->cbuf) - size;
...
{
  bbuf->view += CGA_SCROLL_PAGE;
  size_t end = bbuf->view + CGA_MEMORY_SIZE;
  ...
  switch (c)
    {
      ...
      case '\b':
        ...
        break;
      case '\t':
        for (uint32_t i = 0; i < CGA_TABULATION_SPACES; i++)
          cga_putc (' ');
        break;
...
{
  uint8_t reg = io_read_byte (CGA_PORT_MISC_OUT_READ);
